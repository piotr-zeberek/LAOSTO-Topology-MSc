#ifndef LAOSTO_exts_H
#define LAOSTO_exts_H
...
struct LAOSTO_exts : public System2D
{
    ...
    LAOSTO_exts() : System2D()
    {
        ...
        // Delta matrix
        Hamiltonian delta_mat = Eigen::MatrixXcd::Zero(n_bands, n_bands);
        delta_mat(2, 3) = 1;
        delta_mat(3, 2) = -1;
        delta_mat(4, 5) = 1;
        delta_mat(5, 4) = -1;
...
protected:
    ...
    std::vector<Triplet> Delta_dagger_discrete_ky_hopping_p_triplets(double kx, double y) const;
    ...
    std::vector<Triplet> Delta_dagger_discrete_hopping_xp_triplets(double x, double y) const;
    std::vector<Triplet> Delta_dagger_discrete_hopping_yp_triplets(double x, double y) const;
    // std::vector<Triplet> Delta_dagger_discrete_hopping_pp_triplets(double x, double y) const;
    // std::vector<Triplet> Delta_dagger_discrete_hopping_pm_triplets(double x, double y) const;
...
private:
    ...
    double Delta_SC_exts(double kx, double ky) const
    {
        return delta_SC * 0.5 * (std::cos(kx) + std::cos(ky));
    }
    ...
    double Delta_SC_exts(double kx) const
    {
        return delta_SC * 0.5 * std::cos(kx);
    }
